...
```sh
go get -u 'github.com/denoland/subhosting-go@v0.1.0-alpha.2'
```
...
See the [full list of request options](https://pkg.go.dev/github.com/denoland/subhosting-go/option).
...
We also provide a helper `subhosting.FileParam(reader io.Reader, filename string, contentType string)`
...
```go
params := FooNewParams{
    ID:   subhosting.F("id_xxxx"),
    Data: subhosting.F(FooNewParamsData{
        FirstName: subhosting.F("John"),
    }),
}
